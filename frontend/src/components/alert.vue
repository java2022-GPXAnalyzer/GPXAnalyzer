<template>
  <Transition>
    <div class="alert" v-if="state.mode !== 0">
      <div
        id="alert-1" v-if="state.mode === 1"
...
function hideAlert(){
  state.mode = 0;
  emi.state.showAlertMode = 0;
  emi.isShowAlert = false;
}

watch(
  () => emi.state.showAlertMode,
  (newValue) => {
    if (newValue > 0) {
      state.mode = newValue;
      state.message = emi.alertMessage;
      setTimeout(() => {
        state.mode = 0;
        emi.state.showAlertMode = 0;
        emi.isShowAlert = false;
